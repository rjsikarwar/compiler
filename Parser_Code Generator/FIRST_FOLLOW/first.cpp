...
void add_first(first f_list[], char start[],char term[],int n)
{
	for(int i=0;i<n;i++)
	{
...
/*The following algorithm is used to compute FIRST:
	1> In the first run of the production list, all productions leading to a first term as a terminal i.e. say S->aB
	*  where S is a non-terminal, 'a' is a terminal . All such productions are checked as completed, i.e. no further need to assess them.
	*  Incorporate such terminals in first of corresponding start symbols (non-terminals ).
	2> Repeat the following(3-5) till no. of checked productions match the total no. of productions, note 'chk' variable denotes number of
	*  checked productions.
	3> For every production S->ABC.... ,where A is a non-terminal check whether productions starting with 'A' are complete or not, if 
	* complete then incorporate FIRST(A) - epsilon into FIRST(S) by checking whether they already exist in FIRST(S) or not.
...
//			cout<<"non-term = "<<production[k].deriv[d_it]<<" index = "<<in<<endl;
			...
			{
				add_first(f_list,production[k].start,production[k].deriv[d_it],num_non);
...
void generate_first(char *filename,first *f_list,prod production[],int *num,int *num_prod,char non_term[][20],char start[20])
{
	...
	s.clear();
	f.getline(line,100);				/*The second line in the input file is meant for defining the start symbol in the grammar */
	s<<line;
	s>>junk>>start;
	i=0;
