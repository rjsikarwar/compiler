...
void generate_follow(char *filename,follow_table *fol,first *f_list,prod *production,char *start,char non_term[][20],char terminal[][20],int *num_non1,int *num_prod1,int* num_term1)
{


	int num_non,num_prod,num_term;
	generate_first(filename,f_list,production,&num_non,&num_prod,non_term,start,terminal,&num_term);
	
	follow_chart fol_c[100];
	int fol_c_iter=0,fol_iter=0;
	cout<<endl<<"Production list \n";
